...
required_version = "7.6.3"
current_version = ipywidgets.__version__

if current_version != required_version:
    print(f"V systéme je iná verzia knižnice ipywidgets. Zmením ju na požadovanú {required_version}. Prosím čakajte.")
    
    # Install the required version of ipywidgets
    subprocess.check_output([sys.executable, "-m", "pip", "install", f"ipywidgets=={required_version}", "--quiet", "--disable-pip-version-check"])
    
    # Notify the user to restart the kernel
    print("\nDÔLEŽITÉ: Zmena bola vykonaná. Prosím manuálne reštartujte Jupyter notebook.")
    print("Na toto použite ponuku 'Kernel' -> 'Restart', a potom znovu vykonajte bunku so simuláciou.")

else: # Load your simulation
