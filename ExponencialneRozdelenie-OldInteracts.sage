...
# rozdelenie
dist_name = 'Exponenciálne rozdelenie'
dist_short, m_name, sigma_name = '\\textbf{Exp}', '\\mu', '\\sigma'
...
def zobraz_histogram(bins = 20, n = 1, N = 500, digits = 4, p1 = 1, p2 = 1, ymax=1, 
    ...
    if (data == 'odchýlky (s1)') and (n==1): display(html('odchýlky $s_1$ nie sú pre $n=1$ definované'))
        ...
        if data == 'priemery' and density: 
            g += plot(1/m*exp(-x/m), (x, 0, 7*m), color='gray', linestyle='--', plot_points=30)
        
        g.show(figsize=[6.5,4], frame = True, ymax=ymax, xmin=xmin, xmax=xmax)
